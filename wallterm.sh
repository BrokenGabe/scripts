...
opts="--hide-menubar --hide-toolbar --hide-scrollbar" # xterm,xfce4,gnome terminal options
...
if [[ (${term} = "xfce4-terminal") || (${term} = "gnome-terminal") || (${term} = "xterm") ]]; then
    ${term} ${opts} --hide-borders --title=${title} --geometry=${cols}x${rows}${loc} --command=`${exec}` &
elif [[ ${term} = "konsole" ]]; then
    ${term} --workdir ${dir} ${opts} --noframe --separate -p `TerminalColumns=${cols}` -p `TerminalRows=${rows}` -geometry ${loc} -e `qdbus org.kde.konsole $KONSOLE_DBUS_SESSION setTitle 1 ${title}; ${exec};`
fi
